...
int MainCmds::evalsgflong(int argc, const char* const* argv) {
  Board::initHash();
  ScoreValue::initTables();
  Rand seedRand;
  ...
  try {
    ...
    TCLAP::ValueArg<int> printMaxDepthArg("","print-max-depth","How deep to print",false,1,"DEPTH");
    TCLAP::SwitchArg rawNNArg("","raw-nn","Perform single raw neural net eval");
    TCLAP::SwitchArg logRawNNArg("","log-raw-nn","Perform single raw neural net eval log");
    cmd.add(sgfFileArg);
    cmd.add(moveNumArg);
  ...
  if(logRawNN) {
    ...
    Board::printBoard(cout, board, Board::NULL_LOC, &(hist.moveHistory));
    // buf.result->debugPrint(cout,board);
    cout << "White win: " << buf.result->whiteWinProb << endl;
    cout << "White loss: " << buf.result->whiteLossProb << endl;
    cout << "White noresult: " << buf.result->whiteNoResultProb << endl;
    cout << "White score mean " << buf.result->whiteScoreMean << endl;
    cout << "White score stdev " << sqrt(max(0.0,(double)buf.result->whiteScoreMeanSq - buf.result->whiteScoreMean*buf.result->whiteScoreMean)) << endl;
    
    double whiteWinProb = buf.result->whiteWinProb;
    double whiteLossProb = buf.result->whiteLossProb;
    double whiteNoResultProb = buf.result->whiteNoResultProb;
    double whiteScoreMean = buf.result->whiteScoreMean;
    double whiteScoreStd = sqrt(max(0.0,(double)buf.result->whiteScoreMeanSq - buf.result->whiteScoreMean*buf.result->whiteScoreMean));


    // read a JSON file
    json j;
    std::string jsonStr = sgfFile.substr(0,sgfFile.size() - 4);
    std::ifstream infile(jsonStr + ".json");
    if (infile.is_open())
      infile >> j;
    infile.close();

    json inter;
    inter["whiteWinProb"] = whiteWinProb;
    inter["whiteLossProb"] = whiteLossProb;
    inter["whiteNoResultProb"] = whiteNoResultProb;
    inter["whiteScoreMean"] = whiteScoreMean;
    inter["whiteScoreStd"] = whiteScoreStd;
    std::string moveStr = std::to_string(moveNum);

    if (!j.contains(moveStr)){
      json moveInfo = json::array();
      j[moveStr] = moveInfo;
    }
    j[moveStr].push_back(inter);

    // write prettified JSON to another file
    std::ofstream outfile(jsonStr + ".json");
