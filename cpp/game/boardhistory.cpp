...
static Hash128
getKoHash(const Rules& rules, const Board& board, Player pla, int encorePhase, Hash128 koRecapBlockHash) {
...
// static Hash128 getKoHashAfterMove(const Rules& rules, Hash128 posHashAfterMove, Player pla, int encorePhase, Hash128
// koRecapBlockHashAfterMove) {
//   if(rules.koRule == Rules::KO_SITUATIONAL || rules.koRule == Rules::KO_SIMPLE || encorePhase > 0)
...
BoardHistory::BoardHistory(const BoardHistory& other)
  ...
  std::copy(
    other.wasEverOccupiedOrPlayed, other.wasEverOccupiedOrPlayed + Board::MAX_ARR_SIZE, wasEverOccupiedOrPlayed);
  ...
  std::copy(
    other.secondEncoreStartColors, other.secondEncoreStartColors + Board::MAX_ARR_SIZE, secondEncoreStartColors);
}
...
int BoardHistory::computeNumHandicapStones() const {
  ...
  if(assumeMultipleStartingBlackMovesAreHandicap) {
    ...
    // But if white makes multiple moves in a row after that, then the plays are probably not handicap, someone's
    // setting up a problem position by having black play all moves in a row then white play all moves in a row.
...
void BoardHistory::printDebugInfo(ostream& out, const Board& board) const {
  ...
  out << "Game result " << isGameFinished << " " << PlayerIO::playerToString(winner) << " " << finalWhiteMinusBlackScore
      << " " << isScored << " " << isNoResult << " " << isResignation << endl;
  out << "Last moves ";
  for(int i = 0; i < moveHistory.size(); i++)
...
bool BoardHistory::koHashOccursInHistory(Hash128 koHash, const KoHashTable* rootKoHashTable) const {
  ...
  if(rootKoHashTable != NULL && firstTurnIdxWithKoHistory == rootKoHashTable->firstTurnIdxWithKoHistory) {
...
float BoardHistory::whiteKomiAdjustmentForDraws(double drawEquivalentWinsForWhite) const {
  ...
  // then komi input should be as if it was 7.25, which in a jigo game when jittered by 0.5 gives white 75% wins and 25%
  // losses.
...
float BoardHistory::currentSelfKomi(Player pla, double drawEquivalentWinsForWhite) const {
  float whiteKomiAdjusted =
    whiteBonusScore + whiteHandicapBonusScore + rules.komi + whiteKomiAdjustmentForDraws(drawEquivalentWinsForWhite);
...
int BoardHistory::countAreaScoreWhiteMinusBlack(const Board& board, Color area[Board::MAX_ARR_SIZE]) const {
  ...
  if(rules.taxRule == Rules::TAX_NONE) {
    ...
    board.calculateIndependentLifeArea(
      area, whiteMinusBlackIndependentLifeRegionCount, keepTerritories, keepStones, rules.multiStoneSuicideLegal);
...
bool BoardHistory::isLegal(const Board& board, Loc moveLoc, Player movePla) const {
  ...
  // Special logic for passing... this seems hacky and should probably moved somewhere else
  if(moveLoc == Board::PASS_LOC) {
    bool otherOptions = existsNonPassingLegalMove(board, movePla);

    if(!someoneHasPassed && otherOptions && !playersMatch(movePla, rules.playerThatCanPassFirst))
      return false;
  }
...
bool BoardHistory::isLegalModuloPassing(const Board& board, Loc moveLoc, Player movePla) const {
  ...
  if(encorePhase > 0) {
    if(moveLoc >= 0 && moveLoc < Board::MAX_ARR_SIZE && moveLoc != Board::PASS_LOC) {
      if(
        board.colors[moveLoc] == getOpp(movePla) && koRecapBlocked[moveLoc] && board.getChainSize(moveLoc) == 1 &&
        board.getNumLiberties(moveLoc) == 1)
        return true;
      Loc koCaptureLoc = board.getKoCaptureLoc(moveLoc, movePla);
      if(
        koCaptureLoc != Board::NULL_LOC && koRecapBlocked[koCaptureLoc] &&
        board.colors[koCaptureLoc] == getOpp(movePla))
        return true;
    }
  } else {
...
bool BoardHistory::phaseHasSpightlikeEndingAndPassHistoryClearing() const {
  return encorePhase > 0 || rules.koRule == Rules::KO_SIMPLE || rules.koRule == Rules::KO_SPIGHT;
}
...
bool BoardHistory::wouldBeSpightlikeEndingPass(Player movePla, Hash128 koHashBeforeMove) const {
  if(phaseHasSpightlikeEndingAndPassHistoryClearing()) {
    if(
      movePla == P_BLACK && std::find(hashesBeforeBlackPass.begin(), hashesBeforeBlackPass.end(), koHashBeforeMove) !=
                              hashesBeforeBlackPass.end())
      return true;
    if(
      movePla == P_WHITE && std::find(hashesBeforeWhitePass.begin(), hashesBeforeWhitePass.end(), koHashBeforeMove) !=
                              hashesBeforeWhitePass.end())
      return true;
  }
  return false;
}
...
bool BoardHistory::passWouldEndPhase(const Board& board, Player movePla) const {
  ...
  if(newConsecutiveEndingPassesAfterPass() >= 2 || wouldBeSpightlikeEndingPass(movePla, koHashBeforeMove))
    return true;
  return false;
}

bool BoardHistory::passWouldEndGame(const Board& board, Player movePla) const {
  return passWouldEndPhase(board, movePla) && (rules.scoringRule == Rules::SCORING_AREA ||
                                               (rules.scoringRule == Rules::SCORING_TERRITORY && encorePhase >= 2));
}

bool BoardHistory::isFinalPhase() const {
  return rules.scoringRule == Rules::SCORING_AREA ||
         (rules.scoringRule == Rules::SCORING_TERRITORY && encorePhase >= 2);
}
...
void BoardHistory::makeBoardMoveAssumeLegal(
  ...
  Loc moveLoc,
  Player movePla,
  const KoHashTable* rootKoHashTable) {
...
void BoardHistory::makeBoardMoveAssumeLegal(
  ...
  if(moveLoc != Board::PASS_LOC)
    ...
    // This lifts bans in spight ko rules and lifts 3-fold-repetition checking in the encore for no-resultifying
    // infinite cycles They also clear in simple ko rules for the purpose of no-resulting long cycles. Long cycles with
    // passes do not no-result.
    if(phaseHasSpightlikeEndingAndPassHistoryClearing()) {
      koHashHistory.clear();
      // The first turn idx with history will be the one RESULTING from this move.
      firstTurnIdxWithKoHistory = moveHistory.size() + 1;
  ...
  if(encorePhase > 0 && moveLoc != Board::PASS_LOC) {
    if(board.colors[moveLoc] == getOpp(movePla) && koRecapBlocked[moveLoc]) {
      ...
      // Since we aren't otherwise touching the board, from the board's perspective a player will be moving twice in a
      // row.
  ...
  if(encorePhase <= 0 && rules.koRule != Rules::KO_SIMPLE) {
    ...
    for(int y = 0; y < board.y_size; y++) {
      for(int x = 0; x < board.x_size; x++) {
        Loc loc = Location::getLoc(x, y, board.x_size);
        // Cannot be superko banned if it's not a pseudolegal move in the first place, or we would already ban the move
        // under simple ko.
        if(
          board.colors[loc] != C_EMPTY || board.isIllegalSuicide(loc, nextPla, rules.multiStoneSuicideLegal) ||
          loc == board.ko_loc)
          superKoBanned[loc] = false;
        // Also cannot be superko banned if a stone was never there or played there before AND the move is not suicide,
        // because that means the move results in a new stone there and if no stone was ever there in the past the it
  ...
  if(movePla == P_WHITE && moveLoc != Board::PASS_LOC)
    whiteHasMoved = true;
  if(
    assumeMultipleStartingBlackMovesAreHandicap && !whiteHasMoved && movePla == P_BLACK &&
    rules.whiteHandicapBonusRule != Rules::WHB_ZERO) {
    whiteHandicapBonusScore = computeWhiteHandicapBonus();
  }

  // Update someoneHasPassed
  if(moveLoc == Board::PASS_LOC)
    someoneHasPassed = true;
...
KoHashTable::KoHashTable() : koHashHistorySortedByLowBits(), firstTurnIdxWithKoHistory(0) {
