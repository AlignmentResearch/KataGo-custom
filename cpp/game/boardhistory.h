...
struct BoardHistory {
  ...
  Player winner;
  // Score difference of the game if the game is supposed to have ended now, does NOT take into account
  // whiteKomiAdjustmentForDrawUtility Always an integer or half-integer.
  ...
  // This function should behave gracefully so long as it is pseudolegal (board.isLegal, but also still ok if the move
  // is on board.ko_loc) even if the move violates superko or encore ko recapture prohibitions, or is past when the game
  // is ended. This allows for robustness when this code is being used for analysis or with external data sources.
  // preventEncore artifically prevents any move from entering or advancing the encore phase when using territory
  // scoring.
  void makeBoardMoveAssumeLegal(Board& board, Loc moveLoc, Player movePla, const KoHashTable* rootKoHashTable);
  void makeBoardMoveAssumeLegal(
    Board& board,
    Loc moveLoc,
    Player movePla,
    const KoHashTable* rootKoHashTable,
    bool preventEncore);
  // Make a move with legality checking, but be mostly tolerant and allow moves that can still be handled but that may
  // not technically be legal. This is intended for reading moves from SGFs and such where maybe we're getting moves
  // that were played in a different ruleset than ours. Returns true if successful, false if was illegal even unter
  // tolerant rules.
  ...
  // Compute a hash that takes into account the full situation, the rules, discretized komi, and any immediate ko
  // prohibitions.
  static Hash128 getSituationRulesAndKoHash(
    const Board& board,
