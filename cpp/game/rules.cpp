...
Rules::Rules(int kRule, int sRule, int tRule, bool suic, bool button, int whbRule, bool pOk, float km, Player ppf)
  : koRule(kRule),
    scoringRule(sRule),
    taxRule(tRule),
...
bool Rules::operator==(const Rules& other) const {
  return koRule == other.koRule && scoringRule == other.scoringRule && taxRule == other.taxRule &&
         ...
         komi == other.komi && playerThatCanPassFirst == other.playerThatCanPassFirst;
}
...
int Rules::parseKoRule(const string& s) {
  ...
  else if(s == "SITUATIONAL")
    return Rules::KO_SITUATIONAL;
  else if(s == "SPIGHT")
    return Rules::KO_SPIGHT;
  else
    throw IOError("Rules::parseKoRule: Invalid ko rule: " + s);
}
int Rules::parseScoringRule(const string& s) {
  if(s == "AREA")
    return Rules::SCORING_AREA;
  else if(s == "TERRITORY")
    return Rules::SCORING_TERRITORY;
  else
...
int Rules::parseTaxRule(const string& s) {
  if(s == "NONE")
    ...
    throw IOError("Rules::parseTaxRule: Invalid tax rule: " + s);
}
int Rules::parseWhiteHandicapBonusRule(const string& s) {
  if(s == "0")
    ...
    throw IOError("Rules::parseWhiteHandicapBonusRule: Invalid whiteHandicapBonus rule: " + s);
}

string Rules::writeKoRule(int koRule) {
  if(koRule == Rules::KO_SIMPLE)
    return string("SIMPLE");
  if(koRule == Rules::KO_POSITIONAL)
    return string("POSITIONAL");
  if(koRule == Rules::KO_SITUATIONAL)
    return string("SITUATIONAL");
  if(koRule == Rules::KO_SPIGHT)
    return string("SPIGHT");
  return string("UNKNOWN");
}
string Rules::writeScoringRule(int scoringRule) {
  if(scoringRule == Rules::SCORING_AREA)
    return string("AREA");
  if(scoringRule == Rules::SCORING_TERRITORY)
    return string("TERRITORY");
  return string("UNKNOWN");
}
string Rules::writeTaxRule(int taxRule) {
  if(taxRule == Rules::TAX_NONE)
    return string("NONE");
  if(taxRule == Rules::TAX_SEKI)
    return string("SEKI");
  if(taxRule == Rules::TAX_ALL)
    return string("ALL");
  return string("UNKNOWN");
}
string Rules::writeWhiteHandicapBonusRule(int whiteHandicapBonusRule) {
  if(whiteHandicapBonusRule == Rules::WHB_ZERO)
    return string("0");
  if(whiteHandicapBonusRule == Rules::WHB_N)
    return string("N");
  if(whiteHandicapBonusRule == Rules::WHB_N_MINUS_ONE)
    return string("N-1");
  return string("UNKNOWN");
}

ostream& operator<<(ostream& out, const Rules& rules) {
  out << "ko" << Rules::writeKoRule(rules.koRule) << "score" << Rules::writeScoringRule(rules.scoringRule) << "tax"
      << Rules::writeTaxRule(rules.taxRule) << "sui" << rules.multiStoneSuicideLegal;
  if(rules.hasButton)
...
string Rules::toStringNoKomi() const {
  ostringstream out;
  out << "ko" << Rules::writeKoRule(koRule) << "score" << Rules::writeScoringRule(scoringRule) << "tax"
      << Rules::writeTaxRule(taxRule) << "sui" << multiStoneSuicideLegal;
  if(hasButton)
...
// omitDefaults: Takes up a lot of string space to include stuff, so omit some less common things if matches
// tromp-taylor rules which is the default for parsing and if not otherwise specified
...
Rules Rules::updateRules(const string& k, const string& v, Rules oldRules) {
  ...
  if(key == "ko")
    rules.koRule = Rules::parseKoRule(value);
  else if(key == "score")
    rules.scoringRule = Rules::parseScoringRule(value);
  else if(key == "scoring")
    rules.scoringRule = Rules::parseScoringRule(value);
  else if(key == "tax")
    rules.taxRule = Rules::parseTaxRule(value);
  else if(key == "suicide")
    rules.multiStoneSuicideLegal = Global::stringToBool(value);
  else if(key == "hasButton")
    rules.hasButton = Global::stringToBool(value);
  else if(key == "whiteHandicapBonus")
    rules.whiteHandicapBonusRule = Rules::parseWhiteHandicapBonusRule(value);
  else if(key == "friendlyPassOk")
    rules.friendlyPassOk = Global::stringToBool(value);
  else
...
static Rules parseRulesHelper(const string& sOrig, bool allowKomi) {
  ...
  if(lowercased == "japanese" || lowercased == "korean") {
    ...
    lowercased == "ancientterritory" || lowercased == "ancient-territory" || lowercased == "ancient_territory" ||
    lowercased == "ancient territory") {
    rules.scoringRule = Rules::SCORING_TERRITORY;
    rules.koRule = Rules::KO_SIMPLE;
    rules.taxRule = Rules::TAX_ALL;
    rules.multiStoneSuicideLegal = false;
    ...
    lowercased == "nz" || lowercased == "newzealand" || lowercased == "new zealand" || lowercased == "new-zealand" ||
    lowercased == "new_zealand") {
    rules.scoringRule = Rules::SCORING_AREA;
    rules.koRule = Rules::KO_SITUATIONAL;
    rules.taxRule = Rules::TAX_NONE;
    rules.multiStoneSuicideLegal = true;
    ...
    try {
      ...
      for(json::iterator iter = input.begin(); iter != input.end(); ++iter) {
        ...
        if(key == "ko")
          ...
          rules.taxRule = Rules::parseTaxRule(iter.value().get<string>());
          taxSpecified = true;
        } else if(key == "suicide")
          ...
          komiSpecified = true;
          if(
            rules.komi < Rules::MIN_USER_KOMI || rules.komi > Rules::MAX_USER_KOMI ||
            !Rules::komiIsIntOrHalfInt(rules.komi))
  ...
  // This is more of a legacy internal format, not recommended for users to provide
    ...
    while(true) {
      ...
      if(startsWithAndStrip(s, "ko")) {
        if(startsWithAndStrip(s, "SIMPLE"))
          rules.koRule = Rules::KO_SIMPLE;
        else if(startsWithAndStrip(s, "POSITIONAL"))
          rules.koRule = Rules::KO_POSITIONAL;
        else if(startsWithAndStrip(s, "SITUATIONAL"))
          rules.koRule = Rules::KO_SITUATIONAL;
        else if(startsWithAndStrip(s, "SPIGHT"))
      ...
      if(startsWithAndStrip(s, "scoring")) {
        if(startsWithAndStrip(s, "AREA"))
          rules.scoringRule = Rules::SCORING_AREA;
        else if(startsWithAndStrip(s, "TERRITORY"))
          rules.scoringRule = Rules::SCORING_TERRITORY;
        else
          throw IOError("Could not parse rules: " + sOrig);
        continue;
      ...
      if(startsWithAndStrip(s, "tax")) {
        if(startsWithAndStrip(s, "NONE")) {
          rules.taxRule = Rules::TAX_NONE;
          taxSpecified = true;
        } else if(startsWithAndStrip(s, "SEKI")) {
          rules.taxRule = Rules::TAX_SEKI;
          taxSpecified = true;
        } else if(startsWithAndStrip(s, "ALL")) {
          rules.taxRule = Rules::TAX_ALL;
          taxSpecified = true;
        } else
          throw IOError("Could not parse rules: " + sOrig);
      ...
      if(startsWithAndStrip(s, "whb")) {
        if(startsWithAndStrip(s, "0")) {
          rules.whiteHandicapBonusRule = Rules::WHB_ZERO;
        } else if(startsWithAndStrip(s, "N-1")) {
          rules.whiteHandicapBonusRule = Rules::WHB_N_MINUS_ONE;
        } else if(startsWithAndStrip(s, "N")) {
          rules.whiteHandicapBonusRule = Rules::WHB_N;
        } else
          throw IOError("Could not parse rules: " + sOrig);
        continue;
...
bool Rules::tryParseRules(const string& sOrig, Rules& buf) {
  Rules rules;
  try {
    rules = parseRulesHelper(sOrig, true);
...
bool Rules::tryParseRulesWithoutKomi(const string& sOrig, Rules& buf, float komi) {
  Rules rules;
  try {
    rules = parseRulesHelper(sOrig, false);
  } catch(const StringError&) {
    return false;
  }
  rules.komi = komi;
...
const Hash128
  Rules::ZOBRIST_MULTI_STONE_SUICIDE_HASH =  // Based on sha256 hash of Rules::ZOBRIST_MULTI_STONE_SUICIDE_HASH
