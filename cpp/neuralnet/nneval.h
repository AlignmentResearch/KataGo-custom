...
class NNEvaluator {
 ...
 public:
  ...
  virtual std::string getModelFileName() const;
  virtual std::string getInternalModelName() const;
  virtual Logger* getLogger();
  virtual bool isNeuralNetLess() const;
  virtual int getMaxBatchSize() const;
  virtual int getNumGpus() const;
  virtual int getNumServerThreads() const;
  virtual std::set<int> getGpuIdxs() const;
  virtual int getNNXLen() const;
  virtual int getNNYLen() const;
  virtual enabled_t getUsingFP16Mode() const;
  virtual enabled_t getUsingNHWCMode() const;

  //Check if the loaded neural net supports shorttermError fields
  virtual bool supportsShorttermError() const;

  //Return the "nearest" supported ruleset to desiredRules by this model.
  //Fills supported with true if desiredRules itself was exactly supported, false if some modifications had to be made.
  virtual Rules getSupportedRules(const Rules& desiredRules, bool& supported);
  ...
  virtual void waitForNextNNEvalIfAny();
  ...
  virtual void spawnServerThreads();
  ...
  virtual bool getDoRandomize() const;
  virtual int getDefaultSymmetry() const;
  virtual void setDoRandomize(bool b);
  virtual void setDefaultSymmetry(int s);

  //Some stats
  virtual uint64_t numRowsProcessed() const;
  virtual uint64_t numBatchesProcessed() const;
  virtual double averageProcessedBatchSize() const;
