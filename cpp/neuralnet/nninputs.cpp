...
void NNInputs::fillRowV8(
  ...
  assert(board.x_size <= nnXLen);
  assert(board.y_size <= nnYLen);
  std::fill(rowBin,rowBin+NUM_FEATURES_SPATIAL_V8*nnXLen*nnYLen,false);
  std::fill(rowGlobal,rowGlobal+NUM_FEATURES_GLOBAL_V8,0.0f);
  ...
  if(useNHWC) {
    featureStride = 1;
    posStride = NNInputs::NUM_FEATURES_SPATIAL_V8;
  }
  else {
    featureStride = nnXLen * nnYLen;
    posStride = 1;
  ...
  //We expect that this is hard for a neural net to learn since it depends on the parity of the board size
  //and is very "xor"like.
  //So we provide it as an input, in the form of the following triangular wave with a period of 2 komi points:
  //  ../\........
  //  ./..\.......
  //  /....\..../.
  //  ......\../..
...

  {
    ...
    //We make a wave to distinguish integer vs noninteger komi and the parity of the integer komi. This one we need always.
