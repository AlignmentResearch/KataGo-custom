...
void GameInitializer::initShared(ConfigParser& cfg, Logger& logger) {
  ...
  handicapCompensateKomiProb =
    cfg.contains("handicapCompensateKomiProb") ? cfg.getDouble("handicapCompensateKomiProb", 0.0, 1.0) : 0.0;
  ...
  forkCompensateKomiProb = cfg.contains("forkCompensateKomiProb") ? cfg.getDouble("forkCompensateKomiProb", 0.0, 1.0)
                                                                  : handicapCompensateKomiProb;
  sgfCompensateKomiProb =
    cfg.contains("sgfCompensateKomiProb") ? cfg.getDouble("sgfCompensateKomiProb", 0.0, 1.0) : forkCompensateKomiProb;
...
void GameInitializer::createGame(
  ...
  createGameSharedUnsynchronized(
    board, pla, hist, extraBlackAndKomi, initialPosition, playSettings, otherGameProps, startPosSample);
...
void GameInitializer::createGame(
  ...
  if(drawRandRadius > 1e-30) {
    ...
    if(mean < 0.0 || mean > 1.0)
      throw StringError(
        "GameInitializer: params.drawEquivalentWinsForWhite not within [0,1]: " + Global::doubleToString(mean));
...
void GameInitializer::createGameSharedUnsynchronized(
  ...
  if(initialPosition != NULL) {
    ...
    extraBlackAndKomi = PlayUtils::chooseExtraBlackAndKomi(
      ...
      komiBigStdevProb,
      komiBigStdev,
      sqrt(board.x_size * board.y_size),
      rand);
  ...
  double asymmetricProb = (extraBlackAndKomi.extraBlack > 0) ? playSettings.handicapAsymmetricPlayoutProb
                                                             : playSettings.normalAsymmetricPlayoutProb;
...
bool MatchPairer::getMatchup(BotSpec& botSpecB, BotSpec& botSpecW, Logger& logger) {
  ...
  if(
    blackPriority.size() > 0 && blackPriority.size() == numBots &&
    blackPriority[matchup.first] < blackPriority[matchup.second]) {
...
static void logSearch(Search* bot, Logger& logger, Loc loc, OtherGameProperties otherGameProps) {
  ...
  if(
    otherGameProps.hintLoc != Board::NULL_LOC && otherGameProps.hintTurn == bot->getRootHist().moveHistory.size() &&
...
static SearchLimitsThisMove getSearchLimitsThisMove(
  ...
  if(
    (otherGameProps.hintLoc != Board::NULL_LOC || otherGameProps.isHintFork) &&
...
FinishedGameData* Play::runGame(
  ...
  if(extraBlackAndKomi.makeGameFair) {
    ...
    PlayUtils::adjustKomiToEven(
      botB, botW, board, hist, pla, playSettings.compensateKomiVisits, otherGameProps, gameRand);
    extraBlackAndKomi.komiMean = hist.rules.komi;
    PlayUtils::setKomiWithNoise(extraBlackAndKomi, hist, gameRand);
  } else if(
    (extraBlackAndKomi.extraBlack > 0 || otherGameProps.isFork) && playSettings.fancyKomiVarying &&
  ...
  if(
    playSettings.fancyKomiVarying && botB->getNNEvaluator()->isNeuralNetLess() &&
    ...
    if(proportionOfBoardArea > 0) {
      // Perform the initialization using a different noised komi, to get a bit of opening policy mixing across komi
      {
        ...
        PlayUtils::initializeGameUsingPolicy(
          botB, botW, board, hist, pla, gameRand, doEndGameIfAllPassAlive, proportionOfBoardArea, temperature);
        hist.setKomi(oldKomi);
      }
      bool shouldCompensate = playSettings.compensateAfterPolicyInitProb > 0.0 &&
                              gameRand.nextBool(playSettings.compensateAfterPolicyInitProb);
  ...
  if(
    playSettings.forSelfPlay && !otherGameProps.isHintPos && hist.rules.scoringRule == Rules::SCORING_TERRITORY &&
    hist.encorePhase == 0 && gameRand.nextBool(0.04)) {
    ...
    if(!hist.isGameFinished) {
      ...
      // Since we played out the game a bunch we should get a good mix of stones that were present or not present at the
      // start of the second encore phase if we're going into the second.
  ...
  for(int i = 0; i < maxMovesPerGame; i++) {
    ...
    if(logMoves)
      logger.write(
        "Move " + Global::uint64ToString(hist.moveHistory.size()) + " made: " + Location::toString(loc, board));
    ...
    if(onEachMove != nullptr)
      onEachMove(
        board, hist, pla, loc, historicalMctsWinLossValues, historicalMctsLeads, historicalMctsScoreStdevs, toMoveBot);
  ...
  if(recordFullData) {
    ...
    if(hist.isGameFinished && hist.isNoResult) {
      ...
      // Although in practice actually the training normally weights by having a result or not, so it doesn't matter
      // what we fill.
      ...
      finalValueTargets.score = (float)ScoreValue::whiteScoreDrawAdjust(
        hist.finalWhiteMinusBlackScore, gameData->drawEquivalentWinsForWhite, hist);
      ...
      // Fill full and seki areas
      {
        ...
        for(int i = 0; i < Board::MAX_ARR_SIZE; i++) {
          if(
            independentLifeArea[i] == C_EMPTY &&
            (gameData->finalFullArea[i] == C_BLACK || gameData->finalFullArea[i] == C_WHITE))
    ...
    NNInputs::fillScoring(
      board, gameData->finalOwnership, hist.rules.taxRule == Rules::TAX_ALL, gameData->finalWhiteScoring);
    ...
    vector<double> valueSurpriseByTurn;
    {
      ...
      for(int i = rawNNValues.size() - 1; i >= 0; i--) {
        ...
        if(noResultValue > 1e-100)
          valueSurprise +=
            noResultValue * (log(noResultValue) - log(std::max((double)rawNNValues[i].noResultValue, 1e-100)));
      ...
      if(gameRand.nextBool(0.25)) {
        ...
        if(sp2->hist.isGameFinished)
          ...
          toMoveBot2->getNNEvaluator()->evaluate(
            sp2->board, sp2->hist, sp2->pla, nnInputParams, nnResultBuf, false, false);
    ...
    if(playSettings.estimateLeadProb > 0.0) {
      ...
      for(int turnAfterStart = 0; turnAfterStart < numMoves; turnAfterStart++) {
        ...
        if(
          gameData->targetWeightByTurn[turnAfterStart] > 0 &&
      ...
      for(int i = 0; i < gameData->sidePositions.size(); i++) {
        ...
        if(
          sp->targetWeight > 0 && sp->whiteValueTargets.noResult < 0.3 &&
          gameRand.nextBool(playSettings.estimateLeadProb)) {
          sp->whiteValueTargets.lead = PlayUtils::computeLead(
            botB, botW, sp->board, sp->hist, sp->pla, playSettings.estimateLeadVisits, otherGameProps);
...
void Play::maybeForkGame(
  ...
  if(earlyFork) {
    ...
    moveIdx = finishedGameData->endHist.moveHistory.size() <= 0
                ? 0
                : (int)gameRand.nextUInt(finishedGameData->endHist.moveHistory.size());
  } else {
    ASSERT_UNREACHABLE;
  }
  ...
  for(int i = 0; i < numChoices; i++) {
    ...
    if(
      bestMove == Board::NULL_LOC || (pla == P_WHITE && whiteScore > bestScore) ||
      (pla == P_BLACK && whiteScore < bestScore)) {
      bestMove = loc;
...
void Play::maybeSekiForkGame(
  ...
  if(
    endHist.isGameFinished && endHist.isScored && finishedGameData->startHist.encorePhase < 2 &&
    hasUnownedSpot(finishedGameData)) {
    for(int i = 0; i < 2; i++) {
...
void Play::maybeHintForkGame(
  ...
  if(finishedGameData->startHist.encorePhase != 0)
    return;
  bool hintFork =
    otherGameProps.hintLoc != Board::NULL_LOC && finishedGameData->startBoard.pos_hash == otherGameProps.hintPosHash &&
  ...
  replayGameUpToMove(
    finishedGameData,
    finishedGameData->startHist.moveHistory.size(),
    finishedGameData->startHist.rules,
    board,
    hist,
    pla);
  // Just in case if somehow the game is over now, don't actually do anything
...
FinishedGameData* GameRunner::runGame(
  ...
  if(forkData != NULL) {
    ...
    if(
      initialPosition == NULL && playSettings.sekiForkHackProb > 0 &&
      gameRand.nextBool(playSettings.sekiForkHackProb)) {
  ...
  if(playSettings.forSelfPlay) {
    ...
    gameInit->createGame(
      board, pla, hist, extraBlackAndKomi, params, initialPosition, playSettings, otherGameProps, startPosSample);
  ...
  hist.rules.playerThatCanPassFirst =
    (botSpecB.baseParams.canPassFirst ? P_BLACK : 0) | (botSpecW.baseParams.canPassFirst ? P_WHITE : 0);
  ...
  // In 2% of games, don't autoterminate the game upon all pass alive, to just provide a tiny bit of training data on
  // positions that occur as both players must wrap things up manually, because within the search we don't autoterminate
  // games, meaning that the NN will get called on positions that occur after the game would have been autoterminated.
