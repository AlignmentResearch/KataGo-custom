...
void PlayUtils::placeFixedHandicap(Board& board, int n) {
  ...
  auto s = [&](int xi, int yi) { board.setStone(Location::getLoc(xCoords[xi], yCoords[yi], board.x_size), P_BLACK); };
  if(n == 2) {
    s(0, 1);
    s(1, 0);
  } else if(n == 3) {
...
double PlayUtils::getHackedLCBForWinrate(const Search* search, const AnalysisData& data, Player pla) {
  ...
  double radiusScaleHackFactor =
    ...
    (search->searchParams.winLossUtilityFactor + search->searchParams.staticScoreUtilityFactor +
     search->searchParams.dynamicScoreUtilityFactor + 1.0e-20  // avoid divide by 0
    );
  ...
  double lcb =
    pla == P_WHITE ? winrate - data.radius * radiusScaleHackFactor : winrate + data.radius * radiusScaleHackFactor;
  return lcb;
}
...
static double getNaiveEvenKomiHelper(
  ...
  for(int i = 0; i < 3; i++) {
    std::pair<double, double> result =
      evalKomi(scoreWLCache, botB, botW, board, hist, pla, numVisits, otherGameProps, hist.rules.komi);
    double lead = result.first;
    double winLoss = result.second;

    // If the last shift made stats go the WRONG way, and by a nontrivial amount, then revert half of it and stop
    // immediately.
    if(i > 0) {
      if(
        (lastLead > 0 && lead > lastLead + 5 && winLoss < 0.75) ||
        (lastLead < 0 && lead < lastLead - 5 && winLoss > -0.75) || (lastWinLoss > 0 && winLoss > lastWinLoss + 0.1) ||
  ...
  auto evalWinLoss = [&](double delta) {
    ...
    double winLoss = evalKomi(
                       ...
                       numVisits,
                       otherGameProps,
                       PlayUtils::roundAndClipKomi(newKomi, board, looseClipping))
                       .second;
...
void PlayUtils::adjustKomiToEven(
  ...
  double newKomi =
    getNaiveEvenKomiHelper(scoreWLCache, botB, botW, board, hist, pla, numVisits, otherGameProps, looseClipping);
...
vector<bool> PlayUtils::computeAnticipatedStatusesSimple(const Board& board, const BoardHistory& hist) {
  ...
  for(int i = 0; i < possiblePositionIdxs.size(); i++) {
    ...
    while(moveNum < moves.size() && moveNum < nextIdx) {
      ...
      if(!suc) {
        cerr << endl;
        cerr << board << endl;
        cerr << "SGF Illegal move " << (moveNum + 1) << " for " << PlayerIO::colorToChar(moves[moveNum].pla) << ": "
             << Location::toString(moves[moveNum].loc, board) << endl;
...
Loc PlayUtils::maybeCleanupBeforePass(
  ...
  const bool doCleanupBeforePass = cleanupBeforePass == enabled_t::True ? true
                                   : cleanupBeforePass == enabled_t::False
                                     ? false
  ...
  if(doCleanupBeforePass && moveLoc == Board::PASS_LOC && hist.isFinalPhase() && !hist.hasButton) {
    ...
    for(int y = 0; y < board.y_size; y++) {
      for(int x = 0; x < board.x_size; x++) {
        ...
        if(
          moveLoc == Board::PASS_LOC && board.colors[otherLoc] == C_EMPTY && safeArea[otherLoc] == pla &&
          board.isAdjacentToPla(otherLoc, getOpp(pla)) && hist.isLegal(board, otherLoc, pla)) {
          moveLoc = otherLoc;
        }
      }
    }
...
Loc PlayUtils::maybeFriendlyPass(
  ...
  bool shouldProceed;
  {
    ...
    const BoardHistory& hist = bot->getRootHist();
    const Rules& rules = hist.rules;
    const bool doFriendlyPass = friendlyPass == enabled_t::True ? true
                                : friendlyPass == enabled_t::False
                                  ? false
                                  : (rules.friendlyPassOk == true && rules.scoringRule == Rules::SCORING_AREA);
    shouldProceed =
      (doFriendlyPass && moveLoc != Board::PASS_LOC && rules.scoringRule == Rules::SCORING_AREA &&
       hist.isFinalPhase() && !hist.hasButton && hist.passWouldEndPhase(board, pla) && hist.moveHistory.size() > 0 &&
  ...
  Color area[Board::MAX_ARR_SIZE];
  {
    ...
    cleanBoard.calculateArea(
      area, nonPassAliveStones, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal);
  }
  ...
  // Also, the location that the bot currently wants to move must not be adjacent to an opponent's living stone or a
  // self stone in atari by living stones. This means it's a safe dame or protective fill the bot is trying. It's not a
  // very exhaustive check (since there are also prep connect moves) and maybe has some false positives but it's a free
  // extra check.
  if(
    !foundUnsurroundedSpot && !cleanBoard.isAdjacentToPla(moveLoc, getOpp(pla)) &&
    !cleanBoard.wouldBeCapture(moveLoc, getOpp(pla))) {
    // And also, the bot must rate passing as not significantly worse than playing its move
    SearchParams oldParams = bot->searchParams;
    SearchParams newParams = getNoiselessParams(oldParams, numVisits);
    newParams.playoutDoublingAdvantagePla = C_EMPTY;
    newParams.playoutDoublingAdvantage = 0.0;
    // Conservative pass makes sure we evaluate the opponent's turn as one in which the game is neither finished nor the
    // opponent will be able to end the game via passing.
    newParams.conservativePass = true;
    bot->setParams(newParams);
    ...
    if(
      pla == P_WHITE && valuesAfterPass.utility > valuesAfterMove.utility - 0.1 &&
      ...
      pla == P_BLACK && valuesAfterPass.utility < valuesAfterMove.utility + 0.1 &&
      valuesAfterPass.expectedScore < valuesAfterMove.expectedScore + 0.25 &&
      valuesAfterPass.lead < valuesAfterMove.lead + 0.25) {
      moveLoc = Board::PASS_LOC;
    }
  ...
  std::cerr << "Friendly pass: " << moveLoc << std::endl;
  return moveLoc;
}
