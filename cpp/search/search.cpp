...
ReportedSearchValues::ReportedSearchValues(
  ...
  dynamicScoreValue = ScoreValue::expectedWhiteScoreValue(
    scoreMean, scoreStdev, search.recentScoreCenter, search.searchParams.dynamicScoreCenterScale, search.rootBoard);
  ...
  if(winLossValue > 1.0)
    winLossValue = 1.0;
  if(noResultValue < 0.0)
    noResultValue = 0.0;
  if(noResultValue > 1.0 - abs(winLossValue))
    noResultValue = 1.0 - abs(winLossValue);
...
MoreNodeStats::MoreNodeStats() : stats(), selfUtility(0.0), weightAdjusted(0.0), prevMoveLoc(Board::NULL_LOC) {}
MoreNodeStats::~MoreNodeStats() {}

double Search::getResultUtility(double winLossValue, double noResultValue) const {
  return (winLossValue * searchParams.winLossUtilityFactor + noResultValue * searchParams.noResultUtilityForWhite);
}
...
SearchChildPointer::SearchChildPointer() : data(NULL) {}
...
Search::Search(SearchParams params, NNEvaluator* nnEval, Logger* lg, const string& rSeed, NNEvaluator* oppNNEval)
  : rootPla(P_BLACK),
    rootBoard(),
    rootHistory(),
...
NNEvaluator* Search::getNNEvaluator() const {
  return nnEval__;
}
...
bool Search::makeMove(Loc moveLoc, Player movePla, bool preventEncore) {
  ...
  if(
    (searchParams.conservativePass && rootHistory.passWouldEndGame(rootBoard, rootPla)) ||
...
double Search::getScoreUtilityDiff(double scoreMeanAvg, double scoreMeanSqAvg, double delta) const {
  ...
  double dynamicScoreValueDiff =
    ScoreValue::expectedWhiteScoreValue(
      scoreMean + delta, scoreStdev, recentScoreCenter, searchParams.dynamicScoreCenterScale, rootBoard) -
    ScoreValue::expectedWhiteScoreValue(
      scoreMean, scoreStdev, recentScoreCenter, searchParams.dynamicScoreCenterScale, rootBoard);
  return staticScoreValueDiff * searchParams.staticScoreUtilityFactor +
         dynamicScoreValueDiff * searchParams.dynamicScoreUtilityFactor;
}
...
double Search::getApproxScoreUtilityDerivative(double scoreMean) const {
  double staticScoreValueDerivative =
    ScoreValue::whiteDScoreValueDScoreSmoothNoDrawAdjust(scoreMean, 0.0, 2.0, rootBoard);
  double dynamicScoreValueDerivative = ScoreValue::whiteDScoreValueDScoreSmoothNoDrawAdjust(
    scoreMean, recentScoreCenter, searchParams.dynamicScoreCenterScale, rootBoard);
  return staticScoreValueDerivative * searchParams.staticScoreUtilityFactor +
         dynamicScoreValueDerivative * searchParams.dynamicScoreUtilityFactor;
}
...
double Search::calculateTemperature(double halflife, double earlyValue, double value, int numMoves) const {
...
void Search::runWholeSearch(
...

  {
    ...
    if(
      rootHistory.moveHistory.size() >= 1 &&
      rootHistory.moveHistory[rootHistory.moveHistory.size() - 1].loc == Board::PASS_LOC) {
      if(
        rootHistory.moveHistory.size() >= 3 &&
        rootHistory.moveHistory[rootHistory.moveHistory.size() - 3].loc == Board::PASS_LOC)
  ...
  std::function<void(int)> searchLoop = [this,
                                         &timer,
                                         &numPlayoutsShared,
                                         numNonPlayoutVisits,
                                         &tcMaxTime,
...
void Search::beginSearch(bool pondering) {
  ...
  if(plaThatSearchIsForLastSearch != plaThatSearchIsFor) {
    ...
    if(searchParams.playoutDoublingAdvantage != 0 && searchParams.playoutDoublingAdvantagePla == C_EMPTY)
      clearSearch();
    // If we are doing pattern bonus and the player the search is for changes, clear the search. Recomputing the search
    // tree recursively *would* fix all our utilities, but the problem is the playout distribution will still be
    // matching the old probabilities without a lot of new search, so clearing ensures a better distribution.
  ...
  if(
    ...
    !(searchParams.antiMirror && mirroringPla != C_EMPTY))
  ...
  if(searchParams.avoidRepeatedPatternUtility != 0 || externalPatternBonusTable != nullptr) {
    ...
    if(searchParams.avoidRepeatedPatternUtility != 0) {
      double bonus = plaThatSearchIsFor == P_WHITE ? -searchParams.avoidRepeatedPatternUtility
                                                   : searchParams.avoidRepeatedPatternUtility;
  ...
  if(searchParams.rootSymmetryPruning) {
    ...
    if(rootPruneOnlySymmetries.size() > 0)
      SymmetryHelpers::markDuplicateMoveLocs(
        rootBoard, rootHistory, &rootPruneOnlySymmetries, avoidMoveUntilByLoc, rootSymDupLoc, rootSymmetries);
    else
      SymmetryHelpers::markDuplicateMoveLocs(
        rootBoard, rootHistory, NULL, avoidMoveUntilByLoc, rootSymDupLoc, rootSymmetries);
  } else {
  ...
  if(rootNode == NULL) {
    Loc prevMoveLoc = rootHistory.moveHistory.size() <= 0
                        ? Board::NULL_LOC
                        : rootHistory.moveHistory[rootHistory.moveHistory.size() - 1].loc;
    ...
    if(childrenCapacity > 0 && children != NULL) {
      ...
      // However nothing else should be running at this time and the search hasn't actually started yet, so this is
      // okay.
    ...
    if(
      searchParams.dynamicScoreUtilityFactor != 0 || searchParams.subtreeValueBiasFactor != 0 ||
      patternBonusTable != NULL) {
...
void Search::applyRecursivelyPostOrderMulithreaded(
  const vector<SearchNode*>& nodes,
  std::function<void(SearchNode*, int, bool)>* f) {
...
void Search::applyRecursivelyPostOrderSinglethreadedHelper(
  SearchNode* node,
  int threadIdx,
  std::function<void(SearchNode*, int, bool)>* f) {
  int childrenCapacity;
...
int Search::applyRecursivelyPostOrderMulithreadedHelper(
  SearchNode* node,
  int threadIdx,
  PCG32* rand,
  std::function<void(SearchNode*, int, bool)>* f) {
  ...
  // nodeAge == searchNodeAge means that the node is not fully done, but there is no work for new threads entering this
  // node to do. Sequentially consistent load makes sure that there is a last child whose nodeAge2 is set. The thread
  // that sets it is guaranteed to then see all other nodeAge2s of the children are set. So at least one thread will see
  // all children done, and proceed to handle the parent. We will never thread 1 sees node A done but not B and thread 2
  // sees B done but not A, and both leave and the parent is dropped on the floor.
  ...
  if(numChildren > 0) {
    ...
    // If we didn't even do any of the work, it was all other threads, then it's the responsibility of one of those
    // threads to finish up this node.
    if(lastDeltaDoneByUs <= -1)
      return NOT_DONE_BUT_NO_WORK;
    // If after the latest node we finished, we found nodes that weren't finished that other threads were still working
    // on, then there is nothing for us to do, we leave it up to one of those threads to finish this node too.
    ...
    // At least one thread must reach here, since due to sequential consistency, all threads will agree on the setting
    // order of child->nodeAge2, and there must be a last child whose nodeAge2 was set. The thread that sets the last
    // one will see all other things done at that point, and will reach here.
  }
...
// the table Assumes that this is the only function running concurrently on the search tree
...
void Search::computeRootNNEvaluation(NNResultBuf& nnResultBuf, bool includeOwnerMap) {
  ...
  if(searchParams.playoutDoublingAdvantage != 0) {
    ...
    nnInputParams.playoutDoublingAdvantage =
      (getOpp(pla) == playoutDoublingAdvantagePla ? -searchParams.playoutDoublingAdvantage
                                                  : searchParams.playoutDoublingAdvantage);
  }
  getNNEvaluator()->evaluate(board, hist, pla, nnInputParams, nnResultBuf, skipCache, includeOwnerMap);
}
...
void Search::computeRootValues() {
  ...
  rootBoard.calculateArea(
    rootSafeArea, nonPassAliveStones, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal);
  ...
  if(searchParams.antiMirror) {
    ...
    // If at most of the moves in the game are mirror moves, and many of the recent moves were mirrors, and the last
    // move was a mirror, then the opponent is mirroring.
    if(mirrorCount >= 7.0 + 0.5 * totalCount && mirrorEwms >= 0.45 * totalEwms && lastWasMirror) {
      ...
      if(hist.rules.scoringRule == Rules::SCORING_AREA) {
        ...
        bool blackGetsLastMove =
          (board.x_size % 2 == 1 && board.y_size % 2 == 1) == (numHandicapStones == 0 || numHandicapStones % 2 == 1);
      ...
      if(numHandicapStones > 0 && hist.rules.whiteHandicapBonusRule == Rules::WHB_N_MINUS_ONE)
        blackExtraPoints -= numHandicapStones - 1;
      mirrorAdvantage =
        mirroringPla == P_BLACK ? blackExtraPoints - hist.rules.komi : hist.rules.komi - blackExtraPoints;
    }
...
struct PolicySortEntry {
  ...
  bool operator==(const PolicySortEntry& other) const { return policy == other.policy && pos == other.pos; }
};
...
shared_ptr<NNOutput>* Search::maybeAddPolicyNoiseAndTemp(SearchThread& thread, bool isRoot, NNOutput* oldNNOutput)
  const {
  if(
    searchParams.searchAlgo == SearchParams::SearchAlgorithm::MCTS ||
    (searchParams.searchAlgo == SearchParams::SearchAlgorithm::EMCTS1 && !searchParams.EMCTS1_noiseOppNodes)) {
  ...
  } else if(searchParams.searchAlgo == SearchParams::SearchAlgorithm::EMCTS1 && searchParams.EMCTS1_noiseOppNodes) {
...
bool Search::isAllowedRootMove(Loc moveLoc) const {
  ...
  // To mitigate some of this and save computation, we make it so that at the root, if the last four moves by the
  // opponent were passes, we will never play a move in either player's pass-alive area. In theory this could prune a
  // good move in situations like https://senseis.xmp.net/?1EyeFlaw, but this should be extraordinarly rare,
  if(searchParams.rootPruneUselessMoves && rootHistory.moveHistory.size() > 0 && moveLoc != Board::PASS_LOC) {
    size_t lastIdx = rootHistory.moveHistory.size() - 1;
    Player opp = getOpp(rootPla);
    if(
      lastIdx >= 6 && rootHistory.moveHistory[lastIdx - 0].loc == Board::PASS_LOC &&
      rootHistory.moveHistory[lastIdx - 2].loc == Board::PASS_LOC &&
      rootHistory.moveHistory[lastIdx - 4].loc == Board::PASS_LOC &&
      rootHistory.moveHistory[lastIdx - 6].loc == Board::PASS_LOC && rootHistory.moveHistory[lastIdx - 0].pla == opp &&
      rootHistory.moveHistory[lastIdx - 2].pla == opp && rootHistory.moveHistory[lastIdx - 4].pla == opp &&
...
void Search::downweightBadChildrenAndNormalizeWeight(
  ...
  vector<MoreNodeStats>& statsBuf) const {
...
double Search::getExploreSelectionValue(
  ...
  double exploreComponent = cpuctExploration(totalChildWeight, searchParams) * parentUtilityStdevFactor * nnPolicyProb *
                            sqrt(totalChildWeight + TOTALCHILDWEIGHT_PUCT_OFFSET) / (1.0 + childWeight);
...
double Search::getExploreSelectionValueInverse(
  ...
  double exploreComponentScaling = cpuctExploration(totalChildWeight, searchParams) * parentUtilityStdevFactor *
                                   nnPolicyProb * sqrt(totalChildWeight + TOTALCHILDWEIGHT_PUCT_OFFSET);
...
double Search::getEndingWhiteScoreBonus(const SearchNode& parent, const SearchNode* child) const {
  ...
  if(isAreaIsh) {
    // Areaish scoring - in an effort to keep the game short and slightly discourage pointless territory filling at the
    // end discourage any move that, except in case of ko, is either:
    //  * On a spot that the opponent almost surely owns
    //  * On a spot that the player almost surely owns and it is not adjacent to opponent stones and is not a connection
    //  of non-pass-alive groups.
    // These conditions should still make it so that "cleanup" and dame-filling moves are not discouraged.
    //  * When playing button go, very slightly discourage passing - so that if there are an even number of dame,
    //  filling a dame is still favored over passing.
    if(moveLoc != Board::PASS_LOC && rootBoard.ko_loc == Board::NULL_LOC) {
      ...
      if(plaOwnership <= -extreme)
        ...
        if(
          !rootBoard.isAdjacentToPla(moveLoc, getOpp(rootPla)) &&
    ...
    // Territorish scoring - slightly encourage dame-filling by discouraging passing, so that the player will try to do
    // everything non-point-losing first, like filling dame. Human japanese rules often "want" you to fill the dame so
    // this is a cosmetic adjustment to encourage the neural net to learn to do so in the main phase rather than waiting
    // until the encore. But cosmetically, it's also not great if we just encourage useless threat moves in the
    // opponent's territory to prolong the game. So also discourage those moves except in cases of ko. Also similar to
    // area scoring just to be symmetrical, discourage moves on spots that the player almost surely owns that are not
    // adjacent to opponent stones and are not a connection of non-pass-alive groups.
...
static bool
isMirroringSinceSearchStart(const BoardHistory& rootHistory, const BoardHistory& threadHistory, int skipRecent) {
...
static void maybeApplyAntiMirrorPolicy(
  ...
  if(movePla == getOpp(search->rootPla) && thread->history.moveHistory.size() > 0) {
    ...
    if(moveLoc == mirrorLoc) {
      ...
      bool isDifficult = centerLoc != Board::NULL_LOC && thread->board.colors[centerLoc] == search->mirroringPla &&
                         search->mirrorAdvantage >= -0.5;
      if(isDifficult)
        weight *= 3.0;
    }
  }
...
static void maybeApplyAntiMirrorForcedExplore(
  ...
  if(movePla == mirroringPla && thread->history.moveHistory.size() > 0) {
    ...
    if(moveLoc == mirrorLoc) {
      ...
      if(mirrorLoc == Board::PASS_LOC)
        proportionToDump *=
          (moveLoc == centerLoc ? 0.35 : 0.35 / std::max(1.0, sqrt(search->mirrorCenterSymmetryError)));
      ...
      if(thisChildWeight < proportionToBias * totalChildWeight) {
        childUtility +=
          (parent.nextPla == P_WHITE ? 0.18 : -0.18) * std::max(0.3, 1.0 - 0.7 * parentUtility * parentUtility);
      }
      if(thisChildWeight < 0.5 * proportionToBias * totalChildWeight) {
        childUtility +=
          (parent.nextPla == P_WHITE ? 0.36 : -0.36) * std::max(0.3, 1.0 - 0.7 * parentUtility * parentUtility);
      }
    }
  }
    ...
    if(isDifficult) {
      if(thread->board.isAdjacentToChain(moveLoc, centerLoc)) {
        childUtility += (parent.nextPla == P_WHITE ? 0.75 : -0.75) / (1.0 + thread->board.getNumLiberties(centerLoc)) /
                        std::max(1.0, search->mirrorCenterSymmetryError) *
                        std::max(0.3, 1.0 - 0.7 * parentUtility * parentUtility);
      ...
      // proportionToDump *= (1.0 / (1.0 + sqrt(thread->history.moveHistory.size() -
      // search->rootHistory.moveHistory.size())));
    }
...
double Search::getExploreSelectionValue(
  ...
  // child has been expanded but its thread not yet finished its first visit. It's also possible that we observe
  // childWeight <= 0 even though childVisits >= due to multithreading, the two could be out of sync briefly since they
  ...
  // When multithreading, totalChildWeight could be out of sync with childWeight, so if they provably are, then fix that
  // up
  ...
  if(isDuringSearch && antiMirror) {
    ...
    maybeApplyAntiMirrorForcedExplore(
      childUtility,
      parentUtility,
      moveLoc,
      parentPolicyProbs,
  ...
  return getExploreSelectionValue(
    nnPolicyProb, totalChildWeight, childWeight, childUtility, parentUtilityStdevFactor, parent.nextPla);
}
...
double Search::getFpuValueForChildrenAssumeVisited(
  ...
  double& parentUtility,
  double& parentWeightPerVisit,
  double& parentUtilityStdevFactor) const {
  ...
  if(visits <= 0 || weightSum <= 1)
    ...
    // Make sure we're robust to numerical precision issues or threading desync of these values, so we don't observe
    // negative variance
    if(utilitySqAvg < utilitySq)
      utilitySqAvg = utilitySq;
    parentUtilityStdev = sqrt(std::max(
      0.0,
  ...
  parentUtilityStdevFactor =
    1.0 + searchParams.cpuctUtilityStdevScale * (parentUtilityStdev / searchParams.cpuctUtilityStdevPrior - 1.0);

  if(searchParams.fpuParentWeight > 0.0) {
    parentUtility = searchParams.fpuParentWeight * getUtilityFromNN(*(node.getNNOutput())) +
...
void Search::selectBestChildToDescend(
  ...
  double fpuValue = getFpuValueForChildrenAssumeVisited(
    node, thread.pla, isRoot, policyProbMassVisited, parentUtility, parentWeightPerVisit, parentUtilityStdevFactor);

  std::fill(posesWithChildBuf, posesWithChildBuf + NNPos::MAX_NN_POLICY_SIZE, false);
  bool antiMirror =
    searchParams.antiMirror && mirroringPla != C_EMPTY && isMirroringSinceSearchStart(rootHistory, thread.history, 0);
  ...
  if(searchParams.searchAlgo == SearchParams::SearchAlgorithm::EMCTS1 && node.nextPla != rootNode->nextPla) {
    ...
    // (with an appropriately calcuated temperature).
    {
      ...
      const uint32_t idxChosen =
        chooseIndexWithTemperature(thread.rand, playSelectionValues.data(), playSelectionValues.size(), temperature);
  ...
  // cout << "Descending (" << &node << "): " << maxSelectionValue << ' ' << Location::toString(bestChildMoveLoc,
  // rootBoard) << endl;
}
...
double Search::pruneNoiseWeight(
  ...
  int numChildren,
  double totalChildWeight,
  const double* policyProbsBuf) const {
  if(numChildren <= 1 || totalChildWeight <= 0.00001)
    return totalChildWeight;

  // Children are normally sorted in policy order in KataGo.
  // But this is not guaranteed, because at the root, we might recompute the nnoutput, or when finding the best new
  // child, we have hacks like antiMirror policy and other adjustments. For simplicity, we just consider children in
  // sorted order anyways for this pruning, since it will be close.

  // For any child, if its own utility is lower than the weighted average utility of the children before it, it's
  // downweighted if it exceeds much more than a raw-policy share of the weight.
...
void Search::updateStatsAfterPlayout(SearchNode& node, SearchThread& thread, bool isRoot) {
  ...
  // If we atomically grab a nonzero, then we know another thread must already be doing the work, so we can skip the
  // update ourselves.
  ...
  while(true) {
    ...
    // Otherwise, more threads incremented this more in the meantime. So we need to loop again and add their visits,
    // recomputing again.
...
void Search::recomputeNodeStats(SearchNode& node, SearchThread& thread, int numVisitsToAdd, bool isRoot) {
  ...
  downweightBadChildrenAndNormalizeWeight(
    numGoodChildren, currentTotalChildWeight, desiredTotalChildWeight, amountToSubtract, amountToPrune, statsBuf);
  ...
  // Also add in the direct evaluation of this node.
  {
    ...
    double utility = getResultUtility(winProb - lossProb, noResultProb) + getScoreUtility(scoreMean, scoreMeanSq);
    ...
    if(searchParams.subtreeValueBiasFactor != 0 && node.subtreeValueBiasTableEntry != nullptr) {
      ...
      // This is the amount by which we need to scale desiredSelfWeight such that if the table entry were actually equal
      // to the current difference between the direct eval and the children, we would perform a no-op... unless a noop
      // is actually impossible Then we just take what we can get. desiredSelfWeight *= weightSum / (1.0-biasFactor) /
      // std::max(0.001, (weightSum + desiredSelfWeight - desiredSelfWeight / (1.0-biasFactor)));
    }
...
int Search::runSinglePlayout(SearchThread& thread, double upperBoundVisitsLeft) {
  ...
  int numNodesAdded = 1;
  if(
    searchParams.searchAlgo == SearchParams::SearchAlgorithm::EMCTS1 && thread.lastVisitedNode->nextPla != rootPla &&
    ...
    bool finishedPlayout2 = playoutDescend(thread, *resumeDescentNode, posesWithChildBuf, false);
...
void Search::addLeafValue(
  ...
  double utility = getResultUtility(winLossValue, noResultValue) + getScoreUtility(scoreMean, scoreMeanSq);
  ...
  if(assumeNoExistingWeight) {
    ...
    node.stats.winLossValueAvg.store(
      (node.stats.winLossValueAvg.load(std::memory_order_relaxed) * oldWeightSum + winLossValue * weight) /
        newWeightSum,
      std::memory_order_release);
    node.stats.noResultValueAvg.store(
      (node.stats.noResultValueAvg.load(std::memory_order_relaxed) * oldWeightSum + noResultValue * weight) /
        newWeightSum,
      std::memory_order_release);
    node.stats.scoreMeanAvg.store(
      (node.stats.scoreMeanAvg.load(std::memory_order_relaxed) * oldWeightSum + scoreMean * weight) / newWeightSum,
      std::memory_order_release);
    node.stats.scoreMeanSqAvg.store(
      (node.stats.scoreMeanSqAvg.load(std::memory_order_relaxed) * oldWeightSum + scoreMeanSq * weight) / newWeightSum,
      std::memory_order_release);
    node.stats.leadAvg.store(
      (node.stats.leadAvg.load(std::memory_order_relaxed) * oldWeightSum + lead * weight) / newWeightSum,
      std::memory_order_release);
    node.stats.utilityAvg.store(
      (node.stats.utilityAvg.load(std::memory_order_relaxed) * oldWeightSum + utility * weight) / newWeightSum,
      std::memory_order_release);
    node.stats.utilitySqAvg.store(
      (node.stats.utilitySqAvg.load(std::memory_order_relaxed) * oldWeightSum + utilitySq * weight) / newWeightSum,
      std::memory_order_release);
    node.stats.weightSqSum.store(
      node.stats.weightSqSum.load(std::memory_order_relaxed) + weightSq, std::memory_order_release);
...
void Search::maybeRecomputeExistingNNOutput(SearchThread& thread, SearchNode& node, bool isRoot) {
  ...
  if(isRoot && node.nodeAge.load(std::memory_order_acquire) != searchNodeAge) {
    ...
    // We accept this and tolerate that for a few iterations potentially we will be using the OLD policy - without
    // noise, or without root temperature, etc. Or if we have none of those things, then we'll not end up updating
    // anything except the age, which is okay too.
...
bool Search::initNodeNNOutput(SearchThread& thread, SearchNode& node, bool isRoot, bool skipCache, bool isReInit) {
  bool includeOwnerMap = isRoot || alwaysIncludeOwnerMap;
  bool antiMirrorDifficult = false;
  if(
    searchParams.antiMirror && mirroringPla != C_EMPTY && mirrorAdvantage >= -0.5 &&
    Location::getCenterLoc(thread.board) != Board::NULL_LOC &&
    thread.board.colors[Location::getCenterLoc(thread.board)] == getOpp(rootPla) &&
  ...
  if(searchParams.playoutDoublingAdvantage != 0) {
    Player playoutDoublingAdvantagePla = getPlayoutDoublingAdvantagePla();
    nnInputParams.playoutDoublingAdvantage =
      (getOpp(thread.pla) == playoutDoublingAdvantagePla ? -searchParams.playoutDoublingAdvantage
                                                         : searchParams.playoutDoublingAdvantage);
  }
...
void Search::addCurrentNNOutputAsLeafValue(SearchNode& node, bool assumeNoExistingWeight) {
  ...
  double weight = computeNodeWeight(node);
  addLeafValue(
    node, winProb - lossProb, noResultProb, scoreMean, scoreMeanSq, lead, weight, false, assumeNoExistingWeight);
}
...
bool Search::playoutDescend(
  ...
  if(
    !isRoot && thread.history.isGameFinished &&
    !(searchParams.conservativePass && thread.history.moveHistory.size() == rootHistory.moveHistory.size() + 1 &&
    ...
    // This stops a thread from building a silly number of visits to distort MCTS statistics other threads are stuck on
    // the GPU.
    ...
    if(thread.history.isNoResult) {
      ...
      double lead = 0.0;
      double weight = (searchParams.useUncertainty && getNNEvaluator(node)->supportsShorttermError())
                        ? searchParams.uncertaintyMaxWeight
                        : 1.0;
      addLeafValue(node, winLossValue, noResultValue, scoreMean, scoreMeanSq, lead, weight, true, false);
      return true;
    } else {
      double winLossValue =
        2.0 * ScoreValue::whiteWinsOfWinner(thread.history.winner, searchParams.drawEquivalentWinsForWhite) - 1;
      double noResultValue = 0.0;
      double scoreMean = ScoreValue::whiteScoreDrawAdjust(
        thread.history.finalWhiteMinusBlackScore, searchParams.drawEquivalentWinsForWhite, thread.history);
  ...
  if(nodeState == SearchNode::STATE_UNEVALUATED) {
    // Always attempt to set a new nnOutput. That way, if some GPU is slow and malfunctioning, we don't get blocked by
    // it.
    {
  ...
  while(true) {
    ...
    //(this should only happen either on a bug or where the nnHash doesn't have full legality information or when
    // there's an actual hash collision). Regenerate the neural net call and continue
    if(bestChildIdx >= 0 && !thread.history.isLegal(thread.board, bestChildMoveLoc, thread.pla)) {
      ...
      if(bestChildIdx >= 0) {
        // In THEORY it might still be illegal this time! This would be the case if when we initialized the NN output,
        // we raced against someone reInitializing the output to add dirichlet noise or something, who was doing so
        // based on an older cached nnOutput that still had the illegal move. If so, then just fail this playout and try
        // again.
      ...
      if(searchParams.subtreeValueBiasFactor != 0 && subtreeValueBiasTable != NULL) {
        if(node.prevMoveLoc != Board::NULL_LOC) {
          child->subtreeValueBiasTableEntry =
            subtreeValueBiasTable->get(thread.pla, node.prevMoveLoc, child->prevMoveLoc, thread.board);
        }
      }
