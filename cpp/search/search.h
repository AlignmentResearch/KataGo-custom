...
struct Search {
  ...
  // be banned, for the number of ply of depth that it should be banned.
  ...
  double
    effectiveSearchTimeCarriedOver;  // Effective search time carried over from previous moves due to ponder/tree reuse
 ...
 public:
  ...
  void runWholeSearch(
    ...
    std::function<void()>*
      searchBegun,  // If not null, will be called once search has begun and tree inspection is safe
    bool pondering,
  ...
  bool clipAndScalePlaySelectionValues(
    ...
    const double scaleMaxToAtLeast = 0.0) const;
  ...
  static uint32_t
  chooseIndexWithTemperature(Rand& rand, const double* relativeProbs, int numRelativeProbs, double temperature);
  ...
  std::tuple<std::vector<double>, std::vector<double>> getAverageAndStandardDeviationTreeOwnership(
    double minWeight,
    const SearchNode* node = NULL) const;
 ...
 private:
  ...
  void applyRecursivelyPostOrderMulithreaded(
    const std::vector<SearchNode*>& nodes,
    std::function<void(SearchNode*, int, bool)>* f);
  void applyRecursivelyPostOrderSinglethreadedHelper(
    SearchNode* node,
    int threadIdx,
    std::function<void(SearchNode*, int, bool)>* f);
  int applyRecursivelyPostOrderMulithreadedHelper(
    SearchNode* node,
    int threadIdx,
    PCG32* rand,
    std::function<void(SearchNode*, int, bool)>* f);
  ...
  void maybeRecomputeExistingNNOutput(SearchThread& thread, SearchNode& node, bool isRoot);
