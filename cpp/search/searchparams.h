...
struct SearchParams {
  ...
  double dynamicScoreUtilityFactor;  // Scaling for a [-1,1] "scoreValue" for having more/fewer points, centered at
                                     // recent estimated expected score.
  double dynamicScoreCenterZeroWeight;  // Adjust dynamic score center this proportion of the way towards zero, capped
                                        // at a reasonable amount.
  double dynamicScoreCenterScale;  // Adjust dynamic score scale. 1.0 indicates that score is cared about roughly up to
                                   // board sizeish.
  double
    noResultUtilityForWhite;  // Utility of having a no-result game (simple ko rules or nonterminating territory encore)
  ...
  double cpuctExplorationLog;   // Constant factor on log-scaling exploration, should also scale up linearly with
  ...
  bool useNoisePruning;  // For computation of value, prune out weight that greatly exceeds what is justified by policy
                         // prior
  ...
  double rootDirichletNoiseTotalConcentration;  // Same as alpha * board size, to match alphazero this might be 0.03 *
                                                // 361, total number of balls in the urn
  ...
  int rootNumSymmetriesToSample;      // For the root node, sample this many random symmetries (WITHOUT replacement) and
  ...
  double rootDesiredPerChildVisitsCoeff;  // Funnel sqrt(this * policy prob * total visits) down any given child that
                                          // receives any visits at all at the root
  ...
  double chosenMoveTemperatureHalflife;  // Halflife of decay from early temperature to temperature for the rest of the
                                         // game, scales for board sizes other than 19.
  ...
  Player avoidMYTDaggerHackPla;  // Hacky hack to avoid a particular pattern that gives some KG nets some trouble.
                                 // Should become unnecessary in the future.
  ...
  Player playoutDoublingAdvantagePla;  // Negate playoutDoublingAdvantage when making a move for the opponent of this
                                       // player. If empty, opponent of the root player.

  double avoidRepeatedPatternUtility;  // Have the root player avoid repeating similar shapes, penalizing this much
                                       // utility per instance.

  float
    nnPolicyTemperature;  // Scale neural net policy probabilities by this temperature, applies everywhere in the tree
  bool antiMirror;        // Enable anti-mirroring logic

  double subtreeValueBiasFactor;  // Dynamically adjust neural net utilties based on empirical stats about their errors
                                  // in search
  int32_t
    subtreeValueBiasTableNumShards;  // Number of shards for subtreeValueBiasFactor for initial hash lookup and mutexing
  double subtreeValueBiasFreeProp;   // When a node is no longer part of the relevant search tree, only decay this
                                     // proportion of the weight.
  double subtreeValueBiasWeightExponent;  // When computing empiricial bias, weight subtree results by childvisits to
                                          // this power.
  ...
  int64_t
    maxPlayouts;   // Max number of playouts from the root to think for, not counting earlier playouts from tree reuse
  ...
  // Time control
  double treeReuseCarryOverTimeFactor;  // Assume we gain this much "time" on the next move purely from % tree preserved
                                        // * time spend on that tree.
  ...
  double midgameTurnPeakTime;  // The turn considered to have midgame weight 1.0, rising up from 0.0 in the opening, for
                               // 19x19
  ...
  double obviousMovesPolicySurpriseTolerance;  // What logits of surprise does the search result need to be at most to
                                               // be (1/e) obvious?

  double futileVisitsThreshold;  // If a move would not be able to match this proportion of the max visits move in the
                                 // time or visit or playout cap remaining, prune it.
