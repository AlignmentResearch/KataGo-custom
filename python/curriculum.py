...
# format: victim name, condition for moving on
curriculum_conf = [
...
class Curriculum:
  ...
  def try_move_on(self, adv_stat, victims_input_dir, victims_output_dir):
    if self.finished:
      return

    if not self.cur_victim().check_if_gt(adv_stat):
      return
    ...
    while num_efforts < 10:
      try:
        shutil.copy(os.path.join(victims_input_dir, self.cur_victim().name), victims_output_dir)
        return
      except:
...
def get_game_info(sgf_str: str):
  ...
  # but we really don't need adversary name yet so probably it does not matter
  # if we don't extract more than win rate
  # adv_name = (
...
def recompute_statistics(selfplay_dir, games_for_compute):
  all_sgfs = []
...
if __name__ == '__main__':
  ...
  parser.add_argument('-games-for-compute', type=int, required=True, help='Number of last games number for statistics computation')
  ...
  curriculum = Curriculum(curriculum_conf)
  while True:
    adv_stat = recompute_statistics(args.selfplay_dif, args.games_for_compute)
    curriculum.try_move_on(adv_stat, args.input_models_dir, args.output_models_dir)
    if curriculum.finished:
      print("Curriculum is done. Stopping")
      break
    time.sleep(args.checking_periodicity)
