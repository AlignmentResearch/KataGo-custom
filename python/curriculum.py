...
class Curriculum:
    def __init__(
        ...
        if config_json_file is not None:
            ...
            raise ValueError(
                "Either config or config_json or config_json_file must be provided as a curriculum"
            )
        ...
        for line in config:
            cond = PlayerStat(
                ...
                win_rate=line[1],
                score_diff=line[2],
                score_wo_komi_diff=line[3],
                policy_loss=line[4],
            )
            if not cond.can_be_victim_criteria():
                raise ValueError(
                    "Incorrect victim change criteria for victim '{}': should be a single value enabled".format(
                        line[0]
                    )
                )
    ...
    def try_move_on(self, adv_stat=None, policy_loss=None):
        ...
        while num_efforts < 10:
            try:
                shutil.copy(
                    os.path.join(self.victims_input_dir, self.cur_victim().name),
                    self.victims_output_dir,
                )
                return
            except:
                print(
                    "Cannot copy victim '{}', maybe filesystem problem? Waiting 10 sec...".format(
                        self.cur_victim().name
                    )
                )
                num_efforts += 1
                time.sleep(10)
    ...
    def threaded_loop(self, selfplay_dir, games_for_compute, checking_periodicity):
        while True:
    ...
    def run_thread(self, selfplay_dir, games_for_compute, checking_periodicity):
        thread = Thread(
            target=self.threaded_loop,
...
if __name__ == "__main__":
    ...
    if args.config_json_file is not None:
        ...
        curriculum = Curriculum(
            args.input_models_dir, args.output_models_dir, config=curriculum_conf
        )
